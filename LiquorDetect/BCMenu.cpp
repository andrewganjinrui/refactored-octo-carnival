...
void BCMenu::DrawItem_WinXP (LPDRAWITEMSTRUCT lpDIS)
{
	...
	CRect rect,rect2;
	UINT state = (((BCMenuData*)(lpDIS->itemData))->nFlags);
	COLORREF m_newclrBack= RGB(245,190,239);//GetSysColor(COLOR_3DFACE);
	COLORREF m_clrBack= RGB(170,66,83);// GetSysColor(COLOR_WINDOW);
	...
	if(state & MF_SEPARATOR){
		...
		BOOL standardflag=FALSE,selectedflag=FALSE,disableflag=FALSE;
		BOOL checkflag=FALSE;
		COLORREF crText =  RGB(255,255,255);//GetSysColor(COLOR_MENUTEXT);
		COLORREF crSelect = RGB(126,92,35);//GetSysColor(COLOR_HIGHLIGHT);
